...
const STYLE = `
 .ddz-root{
  /* —— 语义色：映射到 DSH 宿主主题令牌（--dsw-alias-*）。宿主切浅/深色或第三方主题时自动跟随；
        每项带当前浅色兜底，宿主无主题（令牌缺失）时回退历史浅色值，视觉零回归。 —— */
  --dz-blue:var(--dsw-alias-state-business-primary,#4d6bfe);
  --dz-blue-hover:var(--dsw-alias-button-info-hover,#405de0);
  --dz-blue-soft:color-mix(in srgb,var(--dz-blue) 10%,transparent);
  --dz-blue-glow:color-mix(in srgb,var(--dz-blue) 18%,transparent);
  --dz-focus:color-mix(in srgb,var(--dz-blue) 28%,transparent);
  --dz-bg:var(--dsw-alias-bg-base,#f5f7fa);
  --dz-panel:var(--dsw-alias-bg-layer-1,#fff);
  --dz-surface:var(--dsw-alias-bg-base,#fff);
  --dz-table:var(--dsw-alias-bg-module-platform,#f7f8fb);
  --dz-line:var(--dsw-alias-border-l2,#e4e7ed);
  --dz-text:var(--dsw-alias-label-primary,#20242c);
  --dz-dim:var(--dsw-alias-label-secondary,#6f7684);
  --dz-red:var(--dsw-alias-state-error-primary,#c53f4d);
  --dz-red-soft:color-mix(in srgb,var(--dz-red) 14%,transparent);
  --dz-gold:#966813;
  --dz-gold-soft:color-mix(in srgb,var(--dz-gold) 16%,transparent);
  /* 成功/警示绿、橙：作为「成功/警示」的通用语义色，明暗两套主题下保持固定，
     并保证主按钮白字对比度（宿主的 success-green 过亮、白字不达标）。 */
  --dz-green:#2f9e62;
  --dz-green-hover:#26834f;
  --dz-amber:#e6a23c;
  --dz-hover:var(--dsw-alias-interactive-bg-hover-solid,#f2f4f8);
  --dz-disabled:var(--dsw-alias-button-primary-dimmed,#b7becb);
  --dz-btn-ink:var(--dsw-alias-label-primary-foreground,#fff);
  --dz-overlay:var(--dsw-alias-bg-mask-3,rgba(28,32,42,.5));
  /* 物理牌面：不随主题变化（扑克牌白底红黑牌） */
  --dz-card-bg:#fff;
  --dz-card-ink:#20242c;
  --dz-card-line:#d9dde5;
  --dz-card-red:#c53f4d;
 ...
 /* 深色主题：官方深色/第三方深色主题均通过 body[data-ds-dark-theme] 切换，语义令牌已自动跟随；
    这里仅对语义令牌覆盖不到的物理色做深色微调。 */
 body[data-ds-dark-theme] .ddz-root{
  --dz-gold:#e0b24a;                                        /* 金色在深色下提亮 */
  --dz-red:var(--dsw-alias-state-error-secondary,#f25a5a);  /* 红在深色下提亮 */
  --dz-card-line:#3a3d45;                                   /* 深色牌边略亮 */
 }
.ddz-btn{background:var(--dz-blue);border:0;border-radius:9px;color:var(--dz-btn-ink);font-size:14px;line-height:20px;padding:9px 18px;cursor:pointer;font-weight:650;transition:background-color .18s ease,transform .18s ease,box-shadow .18s ease}
.ddz-btn:hover{background:var(--dz-blue-hover);transform:translateY(-1px)}
.ddz-btn:active{transform:translateY(1px)}
 .ddz-btn:focus-visible,.ddz-tab:focus-visible,.ddz-card:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
.ddz-btn:disabled{background:var(--dz-disabled);color:var(--dz-dim);cursor:not-allowed}
.ddz-btn-ghost{background:var(--dz-panel);border:1px solid var(--dz-line);color:var(--dz-text)}
.ddz-btn-ghost:hover{background:var(--dz-hover);color:var(--dz-text)}
.ddz-btn-red{background:var(--dz-red)}
 .ddz-sidebar-entry-host{position:fixed;z-index:2147483000;pointer-events:none}
 .ddz-sidebar-entry{appearance:none;width:100%;height:100%;display:flex;align-items:center;gap:10px;padding:0 14px;border:1px solid var(--dz-line);border-radius:11px;background:var(--dz-panel);color:var(--dz-text);font:inherit;font-size:14px;line-height:20px;text-align:left;cursor:pointer;pointer-events:auto;box-sizing:border-box;transition:background-color .18s ease,border-color .18s ease,box-shadow .18s ease}
 .ddz-sidebar-entry:hover{background:var(--dz-blue-soft);border-color:var(--dz-blue-glow);box-shadow:0 2px 6px rgba(26,32,47,.08)}
 .ddz-sidebar-entry-host.is-active .ddz-sidebar-entry{background:var(--dz-hover);border-color:var(--dz-line);box-shadow:inset 0 0 0 1px var(--dz-line)}
 ...
 .ddz-sidebar-entry:active{background:var(--dz-hover)}
 .ddz-sidebar-entry:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
 .ddz-sidebar-entry-icon{width:24px;height:24px;display:grid;place-items:center;flex:0 0 24px;border-radius:7px;background:var(--dz-blue-soft);color:var(--dz-blue);font-size:15px}
 ...
 .ddz-conversation-popout{position:absolute;top:10px;right:18px;z-index:2;appearance:none;border:1px solid var(--dz-line);border-radius:8px;background:var(--dz-panel);color:var(--dz-dim);padding:6px 9px;font:inherit;font-size:12px;cursor:pointer}
 .ddz-conversation-popout:hover{background:var(--dz-hover);color:var(--dz-text)}
 .ddz-conversation-popout:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
 ...
 .ddz-pip-toolbar{height:42px;flex:0 0 42px;display:flex;align-items:center;justify-content:space-between;padding:0 10px 0 16px;border-bottom:1px solid var(--dz-line);background:var(--dz-panel);color:var(--dz-text);font-size:13px;font-weight:700;cursor:move;user-select:none;touch-action:none}
 ...
 .ddz-pip-toolbar-btn:hover{background:var(--dz-hover);color:var(--dz-text)}
 .ddz-pip-toolbar-btn:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
...
.ddz-corner-close:hover{background:var(--dz-hover);color:var(--dz-text)}
.ddz-corner-close:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
...
.ddz-table-exit:hover{background:var(--dz-hover);color:var(--dz-text)}
.ddz-table-exit:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
...
.ddz-rank{display:inline-block;background:var(--dz-blue-soft);color:var(--dz-blue);border-radius:6px;padding:3px 9px;font-size:12px;font-weight:700}
button.ddz-rank{border:0;cursor:pointer;font-family:inherit;line-height:18px}
button.ddz-rank:hover{background:var(--dz-blue-glow)}
button.ddz-rank:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
.ddz-card{position:relative;width:44px;height:64px;border-radius:7px;background:var(--dz-card-bg);color:var(--dz-card-ink);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;cursor:pointer;user-select:none;border:1px solid var(--dz-card-line);box-shadow:0 1px 3px rgba(26,32,47,.12);transition:transform .14s ease,box-shadow .14s ease}
.ddz-card:hover{transform:translateY(-2px);box-shadow:0 4px 9px rgba(26,32,47,.14)}
.ddz-card.red{color:var(--dz-card-red)}
.ddz-card.sel{transform:translateY(-14px);box-shadow:0 0 0 2px var(--dz-blue),0 5px 10px var(--dz-blue-glow)}
...
.ddz-card-big.sel:hover{transform:translateY(-18px);box-shadow:0 0 0 2px var(--dz-blue),0 5px 10px var(--dz-blue-glow)}
...
.ddz-btn-gold:hover{background:color-mix(in srgb,var(--dz-gold) 78%,#000)}
/* 直接进入机器人对局按钮 */
.ddz-btn-bot{background:var(--dz-green)}
.ddz-btn-bot:hover{background:var(--dz-green-hover)}
.ddz-card-back{cursor:default;background:color-mix(in srgb,var(--dz-card-ink) 4%,var(--dz-card-bg));color:transparent}
...
/* 头像底：固定浅色（同物理牌面，不随主题变色），保证 DeepSeek 蓝/黑 logo 在明暗两套主题下都清晰可辨 */
...
.ddz-turn{color:var(--dz-blue);font-weight:700}
...
.ddz-input{background:var(--dz-panel);border:1px solid var(--dz-line);border-radius:8px;color:var(--dz-text);padding:8px 10px;font-size:14px;width:200px}
.ddz-tab{appearance:none;background:var(--dz-panel);border:1px solid var(--dz-line);color:var(--dz-text);border-radius:10px;padding:14px;cursor:pointer;text-align:left;min-width:0;flex:1;font:inherit}
.ddz-tab:hover{background:var(--dz-blue-soft);border-color:var(--dz-blue-glow)}
.ddz-tab.on{border-color:var(--dz-blue);box-shadow:0 0 0 1px var(--dz-blue);background:var(--dz-blue-soft)}
.ddz-toast{position:fixed;left:50%;top:20px;transform:translateX(-50%);background:var(--dz-panel);color:var(--dz-text);border:1px solid var(--dz-blue);border-radius:10px;padding:10px 18px;font-size:14px;z-index:2147483001;box-shadow:0 8px 12px rgba(26,32,47,.14);animation:ddz-toast-life 2.6s cubic-bezier(.22,1,.36,1) both}
...
.ddz-avatar-button:hover .ddz-avatar{box-shadow:0 0 0 3px var(--dz-blue-glow)}
.ddz-avatar-button:focus-visible,.ddz-icon-btn:focus-visible,.ddz-avatar-option:focus-visible{outline:3px solid var(--dz-focus);outline-offset:2px}
.ddz-avatar-picker{position:absolute;top:calc(100% + 10px);left:0;z-index:4;min-width:168px;padding:10px;border:1px solid var(--dz-line);border-radius:12px;background:var(--dz-panel);box-shadow:0 10px 24px rgba(26,32,47,.14);animation:ddz-picker-in .18s cubic-bezier(.22,1,.36,1) both}
...
.ddz-avatar-option{appearance:none;display:flex;flex-direction:column;align-items:center;gap:5px;padding:6px;border:1px solid transparent;border-radius:9px;background:var(--dz-panel);color:var(--dz-dim);font:inherit;font-size:10px;cursor:pointer}
.ddz-avatar-option:hover{background:var(--dz-blue-soft);border-color:var(--dz-blue-glow)}
.ddz-avatar-option.selected{border-color:var(--dz-blue);background:var(--dz-blue-soft);color:var(--dz-blue);box-shadow:0 0 0 1px var(--dz-blue)}
...
.ddz-icon-btn:hover{background:var(--dz-hover);color:var(--dz-text)}
.ddz-icon-btn svg{width:14px;height:14px;display:block}
.ddz-nickname-editor{display:flex;align-items:center;gap:5px}
.ddz-nickname-input{width:150px;min-width:0;padding:5px 7px;border:1px solid var(--dz-blue);border-radius:6px;background:var(--dz-panel);color:var(--dz-text);font:inherit;font-size:13px;outline:0}
...
.ddz-game-table{min-height:0;gap:14px;padding:24px;background:var(--dz-table);border-color:var(--dz-line);border-radius:18px}
.ddz-top-reveal{display:flex;justify-content:center;align-items:center;min-height:88px}
.ddz-reveal-cards{display:flex;align-items:center;gap:6px;padding:10px 12px;border:1px solid var(--dz-blue-glow);border-radius:12px;background:var(--dz-blue-soft)}
...
.ddz-reveal-back-set{background:var(--dz-table)}
...
.ddz-play-area{position:relative;min-width:0;min-height:96px;width:100%;display:flex;align-items:center;justify-content:center;border:1px solid var(--dz-line);border-radius:14px;background:color-mix(in srgb,var(--dz-panel) 55%,transparent)}
...
.ddz-special-bomb .ddz-card,.ddz-special-rocket .ddz-card{border-color:color-mix(in srgb,var(--dz-gold) 55%,transparent);box-shadow:0 0 0 2px color-mix(in srgb,var(--dz-gold) 12%,transparent),0 6px 14px color-mix(in srgb,var(--dz-gold) 18%,transparent)}
.ddz-special-label{position:absolute;left:8px;bottom:8px;z-index:2;padding:3px 7px;border-radius:999px;background:var(--dz-panel);color:var(--dz-gold);border:1px solid color-mix(in srgb,var(--dz-gold) 40%,transparent);font-size:10px;font-weight:800;line-height:14px;box-shadow:0 2px 5px rgba(26,32,47,.1);animation:ddz-special-label-in .3s .08s both}
.ddz-special-label-rocket{background:var(--dz-blue-soft);color:var(--dz-blue);border-color:var(--dz-blue-glow)}
...
.ddz-action-bubble{position:absolute;left:50%;bottom:calc(100% + 10px);z-index:3;transform:translateX(-50%);padding:8px 12px;border:1px solid var(--dz-blue);border-radius:10px;background:var(--dz-panel);color:var(--dz-text);box-shadow:0 8px 12px rgba(26,32,47,.14);font-size:13px;line-height:18px;white-space:nowrap;animation:ddz-toast-in .22s cubic-bezier(.22,1,.36,1) both}
.ddz-action-bubble::after{content:'';position:absolute;left:50%;bottom:-6px;width:10px;height:10px;background:var(--dz-panel);border-right:1px solid var(--dz-blue);border-bottom:1px solid var(--dz-blue);transform:translateX(-50%) rotate(45deg)}
...
.ddz-play-area-action{position:absolute;left:50%;top:50%;z-index:3;transform:translate(-50%,-50%);font-size:22px;font-weight:800;letter-spacing:.04em;color:var(--dz-blue);line-height:1.2;white-space:nowrap;pointer-events:none;animation:ddz-action-text-in .3s cubic-bezier(.22,1,.36,1) both;text-shadow:0 1px 0 var(--dz-surface),0 2px 6px rgba(26,32,47,.12)}
...
.ddz-action-status{font-size:13px;padding:10px 14px;border-radius:999px;background:var(--dz-panel);border:1px solid var(--dz-line)}
.ddz-countdown{display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:999px;background:var(--dz-gold-soft);color:var(--dz-gold);font-size:11px;font-weight:750;font-variant-numeric:tabular-nums}
.ddz-countdown.urgent{background:var(--dz-red-soft);color:var(--dz-red);animation:ddz-countdown-pulse .8s ease-in-out infinite}
...
.ddz-farmer-badge{background:var(--dz-blue-soft);color:var(--dz-blue)}
...
.ddz-seat-rank{position:absolute;top:-11px;left:24px;z-index:1;display:inline-flex;align-items:center;max-width:74px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;background:var(--dz-blue-soft);color:var(--dz-blue);border-radius:999px;padding:1px 5px;font-size:9px;line-height:13px;font-weight:750;transform:translateX(-50%)}
.ddz-seat-chip{display:flex;align-items:center;gap:9px;padding:6px 12px 6px 6px;border:1px solid var(--dz-line);border-radius:999px;background:color-mix(in srgb,var(--dz-panel) 88%,transparent);box-shadow:0 2px 6px rgba(26,32,47,.08)}
.ddz-seat-chip.is-turn{border-color:var(--dz-blue-glow);animation:ddz-turn-pulse 1.8s ease-in-out infinite}
...
.ddz-card-count{display:inline-block;padding:3px 8px;border-radius:999px;background:var(--dz-hover)}
.ddz-multiplier{display:inline-block;padding:3px 8px;border-radius:999px;background:var(--dz-gold-soft);color:var(--dz-gold);font-weight:750;font-variant-numeric:tabular-nums}
...
.ddz-latency .ddz-latency-dot{width:8px;height:8px;border-radius:50%;background:var(--dz-dim);display:inline-block}
.ddz-latency.good .ddz-latency-dot{background:var(--dz-green)}
.ddz-latency.mid .ddz-latency-dot{background:var(--dz-amber)}
...
.ddz-lobby-latency.good .ddz-latency-dot{background:var(--dz-green)}
.ddz-lobby-latency.mid .ddz-latency-dot{background:var(--dz-amber)}
...
.ddz-dialog{position:fixed;inset:0;z-index:2147483000;background:var(--dz-overlay);display:flex;align-items:center;justify-content:center;animation:ddz-overlay-in .2s ease-out both}
...
.ddz-dialog-code{display:block;background:var(--dz-hover);border:1px solid var(--dz-line);border-radius:8px;padding:8px 10px;font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace;color:var(--dz-text);margin:8px 0 2px;word-break:break-all}
.ddz-rank-table{display:flex;flex-direction:column;gap:8px;margin-top:2px}
.ddz-rank-row{display:flex;flex-direction:column;gap:3px;padding:9px 12px;border:1px solid var(--dz-line);border-radius:10px;background:var(--dz-blue-soft)}
.ddz-rank-row.is-current{border-color:var(--dz-blue);box-shadow:0 0 0 1px var(--dz-blue);background:var(--dz-blue-soft)}
...
.ddz-rank-row-current{font-size:10px;font-weight:700;color:var(--dz-btn-ink);background:var(--dz-blue);border-radius:999px;padding:1px 6px}
...
@keyframes ddz-landlord-reveal{from{opacity:.72;transform:translateY(-5px) scale(.97);box-shadow:0 0 0 0 color-mix(in srgb,var(--dz-blue) 0%,transparent)}50%{box-shadow:0 0 0 5px color-mix(in srgb,var(--dz-blue) 12%,transparent)}to{opacity:1;transform:none;box-shadow:none}}
...
@keyframes ddz-special-play-in{from{opacity:0;transform:translateY(10px) scale(.82);filter:drop-shadow(0 0 0 color-mix(in srgb,var(--dz-red) 0%,transparent))}45%{opacity:1;transform:translateY(-4px) scale(1.04);filter:drop-shadow(0 5px 10px color-mix(in srgb,var(--dz-red) 18%,transparent))}to{opacity:1;transform:none;filter:none}}
@keyframes ddz-bomb-burst{from{opacity:0;transform:translateY(10px) scale(.82);filter:drop-shadow(0 0 0 color-mix(in srgb,var(--dz-gold) 0%,transparent))}35%{opacity:1;transform:translateY(-5px) scale(1.07);filter:drop-shadow(0 0 9px color-mix(in srgb,var(--dz-gold) 45%,transparent))}62%{transform:translateY(1px) scale(.98);filter:drop-shadow(0 0 4px color-mix(in srgb,var(--dz-gold) 22%,transparent))}to{opacity:1;transform:none;filter:none}}
@keyframes ddz-rocket-burst{from{opacity:0;transform:translateY(10px) scale(.82);filter:drop-shadow(0 0 0 color-mix(in srgb,var(--dz-blue) 0%,transparent))}35%{opacity:1;transform:translateY(-6px) scale(1.08);filter:drop-shadow(0 0 11px color-mix(in srgb,var(--dz-blue) 48%,transparent))}65%{transform:translateY(1px) scale(.98);filter:drop-shadow(0 0 5px color-mix(in srgb,var(--dz-blue) 24%,transparent))}to{opacity:1;transform:none;filter:none}}
...
@keyframes ddz-turn-pulse{0%,100%{box-shadow:0 2px 6px rgba(26,32,47,.08)}50%{box-shadow:0 0 0 3px color-mix(in srgb,var(--dz-blue) 12%,transparent),0 3px 8px color-mix(in srgb,var(--dz-blue) 16%,transparent)}}
...
function GameTableShell(props) {
	...
	}, "不叫"), showCountdown && (0, react.createElement)("span", {
		className: "ddz-countdown ddz-action-countdown" + ((remainingSeconds ?? 0) <= 3 ? " urgent" : ""),
		"aria-live": "polite"
	}, `${remainingSeconds}s`)) : (0, react.createElement)("span", { className: "ddz-action-status ddz-dim" }, "等待叫地主…") : view.phase === "robbing" ? isMyTurn ? (0, react.createElement)("div", {
		className: "ddz-row",
		style: { gap: 10 }
	}, (0, react.createElement)("button", {
		className: "ddz-btn",
	...
	}, "不抢"), showCountdown && (0, react.createElement)("span", {
		className: "ddz-countdown ddz-action-countdown" + ((remainingSeconds ?? 0) <= 3 ? " urgent" : ""),
		"aria-live": "polite"
	}, `${remainingSeconds}s`)) : (0, react.createElement)("span", { className: "ddz-action-status ddz-dim" }, "等待抢地主…") : view.phase === "doubling" ? isMyTurn ? (0, react.createElement)("div", {
		className: "ddz-row",
		style: { gap: 10 }
	}, (0, react.createElement)("button", {
		className: "ddz-btn ddz-btn-ghost",
...
function Lobby(props) {
	const { profile, balance, onClaim, claimed, online, matching, matchCount, rescued, onRescue, syncing, lobbyLatency, onRetryConnect, onStartOnline, onStartBot, onCancelMatch, onProfileChange, onClose } = props;
	...
	return (0, react.createElement)(react.Fragment, null, (0, react.createElement)("div", { className: "ddz-body ddz-lobby" }, (0, react.createElement)("div", { className: "ddz-lobby-top" }, (0, react.createElement)("div", { className: "ddz-lobby-profile-stack" }, (0, react.createElement)("div", { className: "ddz-profile ddz-row" }, (0, react.createElement)("div", { className: "ddz-avatar-picker-anchor" }, (0, react.createElement)("button", {
		...
		className: "ddz-btn ddz-btn-ghost",
		onClick: onClose
	}, "最小化")), matching && (0, react.createElement)("div", {
		className: "ddz-row",
		style: {
			marginTop: 10,
		...
		className: "ddz-btn ddz-btn-bot",
		onClick: onStartBot
	}, "直接进入机器人对局")), entryIssue === "low" && (0, react.createElement)("div", { className: "ddz-dim ddz-helper" }, "余额不足该桌门槛，先签到或领救济金"), entryIssue === "high" && (0, react.createElement)("div", { className: "ddz-dim ddz-helper" }, "余额超过该桌上限，请选择更高档桌")), (0, react.createElement)("div", { className: "ddz-lobby-version" }, `斗地主 v${APP_VERSION}`), (0, react.createElement)("div", { className: "ddz-disclaimer" }, "Token 为虚拟货币，仅作娱乐用途，不可兑换任何真实货币或服务（性质类似欢乐豆）"), rankInfoOpen && (0, react.createElement)("div", {
...
function DoudizhuApp() {
	...
	const startBotGame = async () => {
		...
		setResult(null);
		try {
			const r = await joinQueueBot(tableId);
			if (r.status === "matched") {
				setRoomId(r.roomId);
				setMatching(false);
				setScreen("table");
			} else setNotice("机器人对局开启失败，请重试");
		} catch (e) {
			setMatching(false);
